...
const SocialIcon = ({ label }: { label: string }) => (
  <div className="flex size-9 items-center justify-center rounded-full border border-border text-xs font-bold text-muted-foreground">
    {label}
  </div>
)
      ...
      <div className="mx-auto max-w-[1240px] px-6 pb-10 pt-16 lg:px-20">
        <div className="grid grid-cols-2 gap-10 pb-10 md:grid-cols-5">
          ...
          {columns.map((col) => (
            <FooterColumn
              key={col.title}
              col={{ title: col.title, links: col.linkKeys.map((key) => t(key)) }}
            />
          ))}
        </div>
