...
export const MessageRow = ({
  ...
  >
    <div className="flex items-center gap-3 tablet:contents">
      <Avatar
        initials={initials}
        tint={tint}
        size="lg"
      ...
      <div className="min-w-0 flex-1">
        <p className="font-serif type-body-large font-semibold">
          {name}
          <span className="ml-2.5 type-body-small font-medium text-muted-foreground">{relation}</span>
        </p>
        <p className="truncate type-body-small italic text-muted-foreground">
          &ldquo;{quote}&rdquo;
        </p>
      </div>
      <Button variant="ghost" size="icon" className="size-10 shrink-0 rounded-full bg-primary/10 text-primary hover:bg-primary/20 tablet:size-12">
        <Play width={16} height={16} />
      </Button>
    </div>
    <Waveform bars={wave.slice(0, 32)} height={28} className="hidden tablet:flex" />
  </div>
)
