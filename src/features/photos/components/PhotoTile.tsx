...
export const PhotoTile = ({
  ...
  <button
    ...
    onClick={onClick}
    className={cn(
      'group relative mb-3 w-full cursor-pointer overflow-hidden rounded-12 text-left transition-all hover:shadow-md hover:scale-[1.02]',
  ...
  >
    <div
      ...
      style={{ height, background: `linear-gradient(160deg, ${tint}, ${tint}bb)` }}
    >
      ...
      {hasAudio && audioLen && <AudioBadge duration={audioLen} />}
...
const SelectIndicator = ({ selected }: { selected: boolean }) => (
  ...
  >
    {selected && <Check width={12} height={12} className="text-foreground" strokeWidth={2.5} />}
  </div>
)

const FavouriteIndicator = () => (
  <svg className="absolute top-2 right-2" width="16" height="16" viewBox="0 0 24 24" fill="oklch(0.723 0.135 40)" stroke="oklch(0.723 0.135 40)" strokeWidth="1.5">
